...
- **Chế độ Nhập liệu (Typing / Active Recall Mode):** Hệ thống sẽ hiển thị ngẫu nhiên các thẻ yêu cầu người dùng tự gõ lại từ vựng dựa trên định nghĩa (thay vì lật thẻ).
  - **Gợi ý Thông minh (Smart Hint):** Hỗ trợ hiển thị pattern chữ cái thông minh (ví dụ: `H _ _ _ _ d   o _ _` cho "Headed out"). Hệ thống tự động nhận diện cụm từ, hiển thị chữ cái đầu của mỗi từ và thêm các điểm gợi ý dựa trên độ dài từ, giúp kích thích gợi nhớ tối đa. **Đặc biệt, ô nhập liệu sẽ tự động xóa nội dung khi bạn bấm mở Hint để bạn có thể nhập lại từ đầu dựa trên gợi ý.**
  ...
  - **Hệ thống Điểm thưởng (Typing Bonus):** Khi người dùng gõ đúng hoàn toàn mà **không sử dụng bất kỳ gợi ý nào (bao gồm cả Hint hình ảnh và Nghe phát âm trước khi trả lời)**, hệ thống sẽ tặng thêm **+1 điểm** tích lũy (Tổng +3) và cộng thêm chỉ số **Ease Factor** giúp từ vựng đó được giãn lịch ôn tập tối ưu hơn. Một Badge "Stars" sẽ xuất hiện để vinh danh nỗ lực này. Nếu bạn sử dụng "Nghe phát âm" sau khi đã trả lời đúng hoặc khi đáp án đã lộ diện, bạn vẫn nhận được bonus bình thường.
...
VocaBee cung cấp hệ thống thông báo thông minh giúp bạn không bỏ lỡ lộ trình học tập hàng ngày:
- **Nhiệm vụ Từ vựng (Vocabulary Mission):** 
  - Hiển thị banner màu **Vàng/Hổ phách (Amber/Gold)** khi có từ vựng đến hạn ôn tập hoặc cần học từ mới để đạt mục tiêu.
  - Link trực tiếp tới `/review?type=vocab` để tập trung ôn luyện từ vựng.
- **Nhiệm vụ Ngữ pháp (Grammar Mission):** 
  - Hiển thị banner màu **Xanh dương/Tím (Blue/Purple)** với thiết kế premium glassmorphism khi có thẻ ngữ pháp đến hạn.
  - Link trực tiếp tới `/review?type=grammar` để chuyên tâm luyện tập cấu trúc câu.
- **Cơ chế Phân loại:** Hệ thống tự động nhận diện nhu cầu học tập (ôn tập định kỳ hoặc học mới) và gửi thông báo trực quan ngay trên Dashboard để kích thích động lực học tập.
...
- **`app/page.tsx`**: Trang Dashboard chính (Trang chủ). Hiển thị tổng quan tiến độ, danh sách nhiệm vụ hàng ngày (chia riêng Từ vựng và Ngữ pháp), biểu đồ hoạt động và Debug Panel.
...
- **`app/review/`**: Trang Khởi tạo Phiên Ôn Tập (Review Session). Hỗ trợ url parameter `?type=vocab` hoặc `?type=grammar` để ôn tập tách biệt.
...
- **`AddWordForm.tsx` & `AddGrammarForm.tsx`**: Form thêm từ vựng và ngữ pháp mới vào cơ sở dữ liệu. Hỗ trợ đầy đủ các tiêu chuẩn truy cập (Accessibility).
...
- `meaning`: Nghĩa của câu (dùng làm gợi ý chính).
